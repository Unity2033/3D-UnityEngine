...
public class Life_Cycle : MonoBehaviour
{
    ...
    public RawImage URL_Texture;

    void Awake() // 스크립트가 생성될 때 최초로 호출되는 초기화 함수입니다.
    {
        Debug.Log("Awake"); // 스크립트가 비활성화된 상태로 생성되더라도 호출됩니다.    

        StartCoroutine(Load_Texture("http://www.imageclick.com/imgs/main_intro.jpg"));
    }
    ...
    IEnumerator Load_Texture(string URL)
    {
        ...
        {
            URL_Texture.texture = ((DownloadHandlerTexture)request.downloadHandler).texture;
        }
    }
